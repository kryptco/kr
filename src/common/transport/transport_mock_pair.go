...
func (t *ImmediatePairTransport) Read(notifier *socket.Notifier, ps *PairingSecret) (ciphertexts [][]byte, err error) {
	t.Lock()
	defer t.Unlock()
	if t.Keys == nil {
...
func (t *MultiPairTransport) Read(notifier *socket.Notifier, ps *PairingSecret) (ciphertexts [][]byte, err error) {
	t.Lock()
	defer t.Unlock()
