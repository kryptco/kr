...
func TestRemoteUnpair(t *testing.T) {
	...
	go ec.RequestMe(MeRequest{}, false)
...
func TestMeAlert(t *testing.T) {
	transport := &ResponseTransport{T: t, RespondToAlertOnly: true}
	ec := NewTestEnclaveClientShortTimeouts(transport)
	testMeSuccess(t, ec)
}

func TestMeTimeout(t *testing.T) {
	transport := &ResponseTransport{T: t, DoNotRespond: true}
	ec := NewTestEnclaveClientShortTimeouts(transport)

	PairClient(t, ec)
	defer ec.Stop()

	me, err := ec.RequestMe(MeRequest{}, true)
...
func TestSignatureAckDelayWithResponse(t *testing.T) {
	transport := &ResponseTransport{T: t, Ack: true, SendAfterHalfAckDelay: true}
	ec := NewTestEnclaveClientShortTimeouts(transport)
	testSignatureSuccess(t, ec)
}

func TestSignatureAckDelayWithoutResponse(t *testing.T) {
	transport := &ResponseTransport{T: t, Ack: true, SendAfterHalfAckDelay: false}
	ec := NewTestEnclaveClientShortTimeouts(transport)
	resp, _, err := testSignature(t, ec)
	if resp != nil && err != ErrTimeout {
		t.Fatal("expected nil response or timeout")
...
func testSignature(t *testing.T, ec EnclaveClientI) (resp *SignResponse, digest [32]byte, err error) {
	PairClient(t, ec)
	defer ec.Stop()
	...
	signResponse, _, err := ec.RequestSignature(SignRequest{
