...
type EnclaveClientI interface {
	Transport
	Pair(PairingOptions) (pairing *PairingSecret, err error)
	IsPaired() bool
	Unpair()
	Start() (err error)
	Stop() (err error)
	RequestMe(meRequest MeRequest, isPairing bool) (*MeResponse, error)
	GetCachedMe() *Profile
	RequestSignature(SignRequest, func()) (*SignResponse, semver.Version, error)
	RequestGitSignature(GitSignRequest, func()) (*GitSignResponse, semver.Version, error)
	RequestGeneric(Request, func()) (Response, error)
	RequestNoOp() error
}
...
func (ec *EnclaveClient) Pair(pairingOptions PairingOptions) (pairingSecret *PairingSecret, err error) {
	ec.Lock()
	defer ec.Unlock()
...
func (ec *EnclaveClient) generatePairing(pairingOptions PairingOptions) (err error) {
	if ec.pairingSecret != nil {
		ec.unpair(ec.pairingSecret, true)
	}
	...
	pairingSecret, err := GeneratePairingSecret(pairingOptions.WorkstationName)
	if err != nil {
		ec.log.Error(err)
		return
	}
...
func (ec *EnclaveClient) unpair(pairingSecret *PairingSecret, sendUnpairRequest bool) (err error) {
	...
	if sendUnpairRequest {
		func() {
			unpairRequest, err := NewRequest()
			if err != nil {
				ec.log.Error("error creating request:", err)
				return
			}
			unpairRequest.UnpairRequest = &UnpairRequest{}
...
func (ec *EnclaveClient) deactivatePairing(pairingSecret *PairingSecret) (err error) {
	if ec.bt != nil {
...
func (ec *EnclaveClient) getPairingSecret() (pairingSecret *PairingSecret) {
	ec.Lock()
	defer ec.Unlock()
	pairingSecret = ec.pairingSecret
	return
...
func (ec *EnclaveClient) GetCachedMe() (me *Profile) {
	ec.Lock()
	defer ec.Unlock()
	me = ec.cachedMe
	return
...
func (ec *EnclaveClient) postEvent(category string, action string, label *string, value *uint64) {
	...
	if ps != nil {
		...
		if tID != nil {
			go analytics.Analytics{}.PostEvent(*tID, category, action, label, value)
		}
	}
}

func UnpairedEnclaveClient(transport Transport, persister Persister, timeoutsOverride *Timeouts, log *logging.Logger, notifier *socket.Notifier) EnclaveClientI {
...
func (client *EnclaveClient) RequestMe(meSubrequest MeRequest, isPairing bool) (meResponse *MeResponse, err error) {
...
func (client *EnclaveClient) RequestSignature(signRequest SignRequest, onACK func()) (signResponse *SignResponse, enclaveVersion semver.Version, err error) {
...
func (client *EnclaveClient) RequestGitSignature(signRequest GitSignRequest, onACK func()) (signResponse *GitSignResponse, enclaveVersion semver.Version, err error) {
	request, err := NewRequest()
	if err != nil {
		client.log.Error(err)
		return
...
func (client *EnclaveClient) RequestGeneric(request Request, onACK func()) (response Response, err error) {
	start := time.Now()
	err = request.Prepare()
	if err != nil {
		return
...
func (client *EnclaveClient) tryRequest(request Request, timeout time.Duration, alertTimeout time.Duration, alertText string, onACK func()) (callback *callbackT, err error) {
...
//	with this request
func (client *EnclaveClient) sendRequestAndReceiveResponses(pairingSecret *PairingSecret, request Request, cb chan *callbackT, timeout time.Duration, alertFirst bool) (err error) {
	requestJson, err := json.Marshal(request)
	if err != nil {
		err = &ProtoError{err}
		return
...
func (client *EnclaveClient) handleCiphertext(ciphertext []byte, medium string) (err error) {
	...
	if err != nil {
		if err == ErrWrappedKeyUnsupported && client.notifier != nil {
			client.notifier.Notify(append([]byte(Red("You are running an old version of the Krypton app. Please upgrade Krypton on your mobile phone before pairing by visiting get.krypt.co.")), '\r', '\n'))
		}
		err = &ProtoError{err}
		return
	}
...
func (client *EnclaveClient) sendMessage(pairingSecret *PairingSecret, message []byte, queue bool, alertAllowed bool, alertFirst bool) (err error) {
	ciphertext, err := pairingSecret.EncryptMessage(message)
	if err != nil {
		if err == ErrWaitingForKey {
			client.Lock()
...
func (client *EnclaveClient) handleMessage(fromPairing *PairingSecret, message []byte, medium string) (err error) {
