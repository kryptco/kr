...
var logger *logging.Logger = log2.SetupLogging("krssh", logging.WARNING, useSyslog())
...
func sendHostAuth(hostAuth HostAuth) {
	conn, err := HostAuthDial()
	if err != nil {
		os.Stderr.WriteString(Red("Krypton ▶ Could not connect to Krypton daemon. Make sure it is running by typing \"kr restart\".\r\n"))
		return
	}
	defer conn.Close()
...
func startLogger(prefix string, checkForUpdate bool) (r NotificationReader, err error) {
	...
	go func() {
		...
		go func() {
			if checkForUpdate && !silenceWarnings && version.CheckIfUpdateAvailable(logger) {
				os.Stderr.WriteString(Yellow("Krypton ▶ A new version of Krypton is available. Run \"kr upgrade\" to install it. You can view the changelog at https://krypt.co/app/krd_changelog/\r\n"))
			}
		}()
		...
		for {
			...
			case nil:
				...
				if silenceWarnings {
					if strings.Contains(notificationStr, ErrNotPaired.Error()) {
						continue
					}
				}
				if strings.HasPrefix(notificationStr, "[") {
					if prefix != "" && strings.HasPrefix(notificationStr, prefix) {
						...
						if strings.HasPrefix(trimmed, "REJECTED") {
							os.Stderr.WriteString(Red("Krypton ▶ " + ErrRejected.Error() + "\r\n"))
							os.Exit(1)
						}
...
func main() {
	...
	if proxyCommand != nil {
		if proxyHost == nil || *proxyHost == "" {
			os.Stderr.WriteString(Red("No proxy host specified. Please pass proxy host with the '-h' flag to krssh, i.e. 'ProxyCommand krssh -p \"ssh -W %h:%p destination\" -h %h'\r\n"))
			os.Exit(1)
		}
		host = *proxyHost
		...
		if err != nil {
			fatal(Red("could not connect to remote: " + err.Error()))
		}
	}
